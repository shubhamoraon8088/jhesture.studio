    ...
    >
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        {GALLERY_IMAGES.map((img) => (
          <ImageCard key={img.id} data={img} mouseX={mousePos.x} mouseY={mousePos.y} isMobile={isMobile} />
        ))}
      </div>

      {/* Scrolling Marquee */}
      <div className="absolute top-[20%] md:top-[25%] left-0 w-full z-20 overflow-hidden">
        <div className="marquee-container">
          <div className="marquee-content py-2 md:py-0">
            <span className="marquee-item text-[10px] md:text-sm">VIDEO EDITING</span>
            <span className="marquee-item text-[10px] md:text-sm">MOTION GRAPHICS</span>
            <span className="marquee-item text-[10px] md:text-sm">UI ANIMATION</span>
            <span className="marquee-item text-[10px] md:text-sm">AVAILABLE FOR NEW PROJECTS</span>
      ...
      <div className="relative z-20 flex flex-col items-center">
          ...
          >
            ...
            <text
              ...
              style={{
                ...
                fill: 'none',
                stroke: strokeColor,
                strokeWidth: fillLevel > 5 ? (window.innerWidth < 768 ? '1px' : '2px') : '0px',
        ...
        <div className="mt-8 md:mt-12">
          <p className="text-[10px] md:text-sm uppercase tracking-[0.4em] md:tracking-[0.6em] text-brand-muted font-sans font-medium transition-colors text-center px-4">
      ...
      <div className="absolute bottom-10 md:bottom-16 w-full px-6 md:px-12 z-30 flex flex-col md:flex-row justify-between items-center md:items-end gap-8 text-center md:text-left">
        ...
        >
          <p className="text-[10px] md:text-xl uppercase tracking-widest text-brand-muted font-sans font-bold transition-colors">the man. the mind. the vision</p>
          <p className="text-3xl md:text-5xl font-serif italic text-brand-white/90 transition-colors lowercase">shubham toppo</p>
        </motion.div>
        <div className="flex flex-col items-center md:items-end gap-2">
          ...
          <motion.p 
            className="text-lg md:text-2xl uppercase tracking-[0.4em] text-brand-muted font-sans font-medium transition-all duration-500 ease-in-out min-w-[100px]"
            initial={{ opacity: 0 }}
...
const ImageCard: React.FC<{ data: GalleryImage; mouseX: number; mouseY: number; isMobile: boolean }> = ({ data, mouseX, mouseY, isMobile }) => {
  ...
  return (
    <div
      ...
      style={{
        top: data.top,
        left: data.left,
        width: isMobile ? `calc(${data.width} * 0.6)` : data.width,
